...
@Route("collaborative-master-detail/:sampleAddressID?/:action?(edit)")
@Menu(order = 9, icon = LineAwesomeIconUrl.COLUMNS_SOLID)
@RolesAllowed("USER")
public class CollaborativeMasterDetailView extends Div implements BeforeEnterObserver {
    ...
    public CollaborativeMasterDetailView(SampleAddressService sampleAddressService) {
        ...
        grid.setItems(query -> sampleAddressService.list(VaadinSpringDataHelpers.toSpringPageRequest(query)).stream());
        ...
        save.addClickListener(e -> {
            try {
                ...
                sampleAddressService.save(this.sampleAddress);
                clearForm();
                refreshGrid();
