package com.acme.fw.vaadin.views.masterdetail;
...
@Uses(Icon.class)
public class UserBrowserView extends Div implements BeforeEnterObserver {
    ...
    public UserBrowserView(UserService userService) {
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                clearForm();
                UI.getCurrent().navigate(UserBrowserView.class);
            }
        });
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (userId.isPresent()) {
            ...
            if (samplePersonFromBackend.isPresent()) {
                ...
                Notification.show(
                        String.format("The requested samplePerson was not found, ID = %s", userId.get()), 3000,
                        Notification.Position.BOTTOM_START);
                // when a row is selected but the data is no longer available,
                // refresh grid
                refreshGrid();
                event.forwardTo(UserBrowserView.class);
            }
        }
    }
    ...
    private void createEditorLayout(SplitLayout splitLayout) {
        ...
        formLayout.add(login, name, active);
